...
# improved Static Estimation for MidEndGame
def getImprovedStatEstForMidgameEndgame(board,depth):
    ...
    if blackMoves == 0:
        return float('inf')
    potentialOrRealWhiteMillsWeight = getpotentialOrRealMillsWeights(board,'W')
    potentialOrRealBlackMillsWeight = getpotentialOrRealMillsWeights(board,'B')   
    return 1000*(potentialOrRealWhiteMillsWeight - potentialOrRealBlackMillsWeight) + 1000000*(whiteCount-blackCount) - 100*blackMoves


millPositions = [[0,2,4],[1,3,5],[1,8,17],[3,7,14],[5,6,11],[6,7,8],[9,10,11],[9,12,15],[10,13,16],[11,14,17],
    [12,13,14],[15,16,17]]
millsLength = len(millPositions)
rangeMillPs = range(millsLength)
iterateMills= range(3)
# find mill weight where there are atleast 2 pieces of the same color
def getpotentialOrRealMillsWeights(board,color):
    prmills = 0
    for i in rangeMillPs:
        pieceCount = 0
        occCount = 0
        for j in iterateMills:
            curColor = board[millPositions[i][j]]
            if curColor == color:
...
def abMaxMin(board,alpha,beta,height,depth,staticEst,generateMoves,staticEstCounter,startime,timer)->Solution:
    solution = []
    if height == 0:# or (time.time() - startime) > timer
        #staticEstCounter[0] = staticEstCounter[0] + 1
        return Solution(staticEst(board,depth),board)
    else:
        v = -float('inf')
        openingMoves = generateMoves(board)
        for i in range(len(openingMoves)):
            minMaxVal=abMinMax(openingMoves[i],alpha,beta,height-1,depth+1,staticEst,generateMoves,staticEstCounter,startime,timer)
            if minMaxVal.val > v:
                v = minMaxVal.val
...
def abMinMax(board,alpha,beta,height,depth,staticEst,generateMoves,staticEstCounter,startime,timer)->Solution:
    solution = []
    if height == 0:# or (time.time() - startime) > timer
        #staticEstCounter[0] = staticEstCounter[0] + 1
        return Solution(staticEst(board,depth),board)
        ...
        for i in range(len(openingMoves)):
            ...
            maxMinVal=abMaxMin(reSwapBoard,alpha,beta,height-1,depth+1,staticEst,generateMoves,staticEstCounter,startime,timer)
            if maxMinVal.val < v:
                v = maxMinVal.val
...
if __name__ == "__main__":
    ...
    swappedIB = swapPieces(inpBoard)
    fig, ax = plt.subplots(1,2, figsize=(10, 5))           
    staticEstCounter = [0]    
    res = abMaxMin(swappedIB,-float('inf'),float('inf'),height,0,getImprovedStatEstForMidgameEndgame,generateMovesMidgameEndgame,staticEstCounter,starttime,40) 
    swappedFB =  swapPieces(res.state)
    finalBoard = ''.join(swappedFB)
